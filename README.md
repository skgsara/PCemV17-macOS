...
- `configure.ac` : recognize the Apple Silicon `arm` host CPU as arm64
- `src/Makefile.am` : SDL2 include path via `sdl2-config`; enable the
  `off64_t`/`fopen64` fallback defines on macOS
- `src/codegen_allocator.c`, `src/codegen_allocator.h` : allocate dynarec memory
  with `MAP_JIT` and toggle it between writable and executable with
  `pthread_jit_write_protect_np`, as required on Apple Silicon
- `src/386_dynarec.c` : toggle JIT memory permissions around recompiled block execution
- `src/wx-sdl2-display.c` : marshal window title updates to the main thread
