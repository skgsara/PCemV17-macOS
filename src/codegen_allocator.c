...
void codegen_allocator_init()
{
        ...
        /* Apple Silicon requires MAP_JIT and pthread_jit_write_protect_np. */
        mem_block_alloc = mmap(0, MEM_BLOCK_NR * MEM_BLOCK_SIZE, PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE|MAP_JIT, 0, 0);
        if (mem_block_alloc != MAP_FAILED)
...
void codegen_allocator_jit_writable()
{
...
void codegen_allocator_jit_executable()
{
#if defined(__APPLE__) && defined(__aarch64__)
