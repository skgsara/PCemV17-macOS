...
#ifdef __APPLE__
static int set_window_title_on_main_thread(void* data)
{
        char* title = data;
        if (window)
...
void sdl_set_window_title(const char* title) {
        if (!window || is_fullscreen())
                return;
#ifdef __APPLE__
        /*This is called from the emulation thread, but Cocoa requires
          window changes to happen on the main thread, so queue the
          update to the wx event loop*/
        char* title_copy = strdup(title);
        if (title_copy)
                wx_callback(ghwnd, set_window_title_on_main_thread, title_copy);
#else
        SDL_SetWindowTitle(window, title);
#endif
}
